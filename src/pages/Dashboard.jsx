...
const Dashboard = ({ expenseEntries, incomeEntries, categoryEntries, budgets = [], onOpenAddForm, selectedMonth, onMonthChange, budgetCycle, onBudgetCycleChange }) => {
  ...
  const matchesRange = (dateValue) => {
    ...
    if (rangeKey === 'month' || !rangeKey) return true
  ...
  const orderedMetricButtons = [...metricCards].sort((a, b) => metricOrder.indexOf(a.key) - metricOrder.indexOf(b.key))

  const trendData = useMemo(() => buildTrendData({ incomeEntries: rangeFilteredIncome, expenseEntries: rangeFilteredExpenses, selectedMonth, rangeKey }), [rangeFilteredExpenses, rangeFilteredIncome, rangeKey, selectedMonth])
  const categoryBreakdownData = useMemo(() => buildCategoryBreakdownData({ expenseEntries: rangeFilteredExpenses, selectedMonth, rangeKey }), [rangeFilteredExpenses, rangeKey, selectedMonth])
  ...
  const dailyStatusMessage = !budgetMetrics ? 'Select a category to see its daily budget.' : budgetMetrics.status === 'over_budget' ? `You are ${formatCurrency(Math.abs(budgetMetrics.remainingBudget))} over this budget.` : budgetMetrics.status === 'not_started' ? `Budget starts ${formatBudgetDate(budgetMetrics.startDate)}.` : budgetMetrics.status === 'completed' ? 'Budget period ended.' : budgetMetrics.dailyStatus === 'over' ? `You've exceeded today's recommended spending by ${formatCurrency(Math.abs(budgetMetrics.dailyDifference))}.` : budgetMetrics.dailyStatus === 'on_track' ? `You're at today's recommended spending of ${formatCurrency(budgetMetrics.recommendedDailySpend)}.` : `You can spend up to ${formatCurrency(budgetMetrics.recommendedDailySpend)} today.`
  ...
  return (
    <section>
      ...
      <div className="dashboard-filter-bar">
        <div className="dashboard-filter-group"><label htmlFor="dashboard-category-filter">View</label><select id="dashboard-category-filter" value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)}><option value="all">All categories</option>{categoryEntries.map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select></div>
      ...
      <div className="daily-budget-section">
        <div className="daily-budget-header"><div><p className="chart-eyebrow">Daily budgeting</p><h2>How much can I spend?</h2><p className="daily-budget-description">Choose a category and period to calculate a safe daily spending amount from your remaining budget.</p></div>{budgetMetrics?.status === 'over_budget' && <span className="daily-budget-status status-danger">Over budget</span>}{budgetMetrics?.status === 'completed' && <span className="daily-budget-status status-neutral">Period ended</span>}{budgetMetrics?.status === 'active' && budgetMetrics.dailyStatus === 'under' && <span className="daily-budget-status status-good">On track</span>}</div>
        <div className="daily-budget-controls">
          <div className="dashboard-filter-group"><label htmlFor="budget-category-select">Category</label><select id="budget-category-select" value={selectedCategory === 'all' ? (budgetCategory?.name ?? '') : selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)}>{budgetCategoryOptions.map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select></div>
        ...
        {!budgetCategory ? <div className="daily-budget-empty">Create a budget category first to use daily budgeting.</div> : !budgetMetrics?.hasBudget ? <div className="daily-budget-empty">No budget amount is available for <strong>{budgetCategory.name}</strong>. Add a category budget to start tracking daily spending.</div> : <>
          <div className="daily-budget-grid">
            <div className="daily-budget-primary"><span>Recommended daily spending</span>{budgetMetrics.status === 'over_budget' ? <strong className="amount-negative">Over budget</strong> : budgetMetrics.status === 'completed' ? <strong>—</strong> : <strong>{formatCurrency(budgetMetrics.currentDailyAllowance)}<small>/day</small></strong>}<p>{dailyStatusMessage}</p></div>
            <div className="daily-budget-stat"><span>Total budget</span><strong>{formatCurrency(budgetMetrics.budgetAmount)}</strong></div><div className="daily-budget-stat"><span>Total spent</span><strong>{formatCurrency(budgetMetrics.spent)}</strong></div><div className="daily-budget-stat"><span>Remaining</span><strong className={budgetMetrics.remainingBudget < 0 ? 'amount-negative' : ''}>{formatCurrency(budgetMetrics.remainingBudget)}</strong></div><div className="daily-budget-stat"><span>Days elapsed</span><strong>{budgetMetrics.daysElapsed}</strong></div><div className="daily-budget-stat"><span>Days remaining</span><strong>{budgetMetrics.remainingDays}</strong></div><div className="daily-budget-stat"><span>Original daily allowance</span><strong>{formatCurrency(budgetMetrics.originalDailyAllowance)}</strong></div><div className="daily-budget-stat"><span>Today's spending</span><strong>{formatCurrency(budgetMetrics.todaysExpenses)}</strong></div>
          </div>
          <div className="daily-budget-progress-area"><div className="daily-budget-progress-labels"><span>{budgetCategory.name} · {budgetPeriodLabel}</span><strong>{Math.min(100, Math.max(0, budgetMetrics.percentageUsed)).toFixed(1)}% used</strong></div><div className="daily-budget-progress-track" role="progressbar" aria-valuenow={Math.min(100, Math.max(0, budgetMetrics.percentageUsed))} aria-valuemin="0" aria-valuemax="100" aria-label={`${budgetCategory.name} budget used`}><div className="daily-budget-progress-fill" style={{ width: `${Math.min(100, Math.max(0, budgetMetrics.percentageUsed))}%` }} /></div><div className="daily-budget-progress-meta"><span>{formatCurrency(budgetMetrics.spent)} spent</span><span>{formatBudgetDate(budgetMetrics.startDate)} – {formatBudgetDate(budgetMetrics.endDate)}</span><span>{formatCurrency(budgetMetrics.budgetAmount)} budget</span></div></div>
        </>}
      </div>
