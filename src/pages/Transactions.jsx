...
import { faXmark, faTrashCan, faPenToSquare, faFilter, faReceipt, faArrowRightArrowLeft } from '@fortawesome/free-solid-svg-icons'
...
const Transactions = ({ expenseEntries, categoryEntries = [], accounts = [], transfers = [], onOpenAddForm, selectedMonth, onMonthChange, budgetCycle, onBudgetCycleChange, onUpdateExpenseEntry, onDeleteExpenseEntry, onDeleteTransfer }) => {
  ...
  const sortedTransactions = useMemo(() => {
    const rows = [
      ...filteredExpenses.map(entry => ({ type: 'expense', entry, date: new Date(entry.date), createdAt: new Date(entry.createdAt ?? entry.date), description: entry.title, account: getAccountName(entry), amount: Number(entry.amount ?? 0) })),
      ...filteredTransfers.map(entry => ({ type: 'transfer', entry, date: new Date(entry.date), createdAt: new Date(entry.createdAt ?? entry.date), description: entry.fromAccount + ' → ' + entry.toAccount, account: entry.fromAccount + ' → ' + entry.toAccount, amount: Number(entry.amount ?? 0) }))
    ]
    return rows.sort((a, b) => {
      let comparison = 0
      if (sortField === 'date') comparison = a.date.getTime() - b.date.getTime()
      else if (sortField === 'createdAt') comparison = a.createdAt.getTime() - b.createdAt.getTime()
  ...
  const handleSort = (field) => {
    if (sortField === field) setSortDirection(previous => previous === 'asc' ? 'desc' : 'asc')
      ...
      setSortField(field)
      setSortDirection(field === 'date' || field === 'createdAt' ? 'desc' : 'asc')
    }
  }

  const formatAddedTime = (value) => value ? new Date(value).toLocaleString('en-PH', { dateStyle: 'medium', timeStyle: 'short' }) : 'Not recorded'
    ...
    <div>
      ...
      <div className="transactions-filter-bar">
        <div className="transactions-filter-header">
          <div className="transactions-filter-heading"><span className="transactions-filter-icon" aria-hidden="true"><FontAwesomeIcon icon={faFilter} /></span><div><strong>Filter transactions</strong><span>{transactionCount} {transactionCount === 1 ? 'transaction' : 'transactions'} shown</span></div></div>
      ...
      <div className="transactions-table-wrapper">
        {transactionCount === 0 ? <p className="empty-state">{hasFilters ? 'No transactions match your filters.' : 'No transactions have been recorded yet.'}</p> : <table className="transactions-table">
          <thead><tr><th><button type="button" className="transaction-sort-button" onClick={() => handleSort('date')}>Date{sortIndicator('date')}</button></th><th><button type="button" className="transaction-sort-button" onClick={() => handleSort('createdAt')}>Time added{sortIndicator('createdAt')}</button></th><th><button type="button" className="transaction-sort-button" onClick={() => handleSort('type')}>Type{sortIndicator('type')}</button></th><th><button type="button" className="transaction-sort-button" onClick={() => handleSort('description')}>Description{sortIndicator('description')}</button></th><th><button type="button" className="transaction-sort-button" onClick={() => handleSort('account')}>Account{sortIndicator('account')}</button></th><th className="transactions-amount"><button type="button" className="transaction-sort-button" onClick={() => handleSort('amount')}>Amount{sortIndicator('amount')}</button></th><th className="transactions-actions">Actions</th></tr></thead>
          <tbody>
            {sortedTransactions.map(({ type, entry }) => {
              if (type === 'expense') {
                const accountName = getAccountName(entry)
                return <tr key={`expense-${entry.id}`}><td className="transaction-date">{entry.date.toLocaleDateString()}</td><td className="transaction-date">{formatAddedTime(entry.createdAt ?? entry.date)}</td><td><span className="transaction-type transaction-type-expense"><FontAwesomeIcon icon={faReceipt} aria-hidden="true" />Expense</span></td><td className="transaction-title-cell">{entry.title}<small>{entry.category || 'Uncategorized'}</small></td><td className="transaction-account-cell">{accountName}</td><td className="transactions-amount transaction-expense-amount">-₱{Number(entry.amount).toFixed(2)}</td><td className="transactions-actions"><div className="transaction-row-actions"><button type="button" className="transaction-action-button" onClick={() => handleEditStart(entry)}><FontAwesomeIcon icon={faPenToSquare} aria-hidden="true" />Edit</button><button type="button" className="transaction-action-button transaction-action-delete" onClick={() => setPendingDeleteExpense(entry)}><FontAwesomeIcon icon={faTrashCan} aria-hidden="true" />Delete</button></div></td></tr>
              }
              return <tr key={`transfer-${entry.id}`}><td className="transaction-date">{new Date(entry.date).toLocaleDateString()}</td><td className="transaction-date">{formatAddedTime(entry.createdAt ?? entry.date)}</td><td><span className="transaction-type transaction-type-transfer"><FontAwesomeIcon icon={faArrowRightArrowLeft} aria-hidden="true" />Transfer</span></td><td className="transaction-title-cell">{entry.fromAccount} → {entry.toAccount}<small>Account transfer</small></td><td className="transaction-account-cell">{entry.fromAccount} → {entry.toAccount}</td><td className="transactions-amount transaction-transfer-amount">₱{Number(entry.amount).toFixed(2)}</td><td className="transactions-actions"><div className="transaction-row-actions"><span className="transaction-logged">Logged</span><button type="button" className="transaction-action-button transaction-action-delete" onClick={() => setPendingDeleteTransfer(entry)}><FontAwesomeIcon icon={faTrashCan} aria-hidden="true" />Delete</button></div></td></tr>
            })}
          </tbody>
        </table>}
      </div>
      ...
      {selectedTransaction && selectedEntry && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="transaction-details-title" onClick={() => setSelectedTransaction(null)}><div className="transaction-details-card" onClick={event => event.stopPropagation()}>
        <div className="transaction-details-header"><div><h3 id="transaction-details-title">Transaction details</h3><p>{selectedIsExpense ? 'Expense' : 'Account transfer'}</p></div><button className="add-form-close" type="button" onClick={() => setSelectedTransaction(null)} aria-label="Close transaction details"><FontAwesomeIcon icon={faXmark} aria-hidden="true" /></button></div>
      ...
      {pendingDeleteTransfer && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="delete-transfer-title"><div className="add-form edit-form"><div className="add-form-header"><h3 id="delete-transfer-title">Delete transfer?</h3><button className="add-form-close" type="button" onClick={() => setPendingDeleteTransfer(null)} aria-label="Close transfer deletion confirmation"><FontAwesomeIcon icon={faXmark} aria-hidden="true" /></button></div>
        ...
        <div className="budget-card-actions edit-action-row"><button type="button" className="budget-card-button budget-card-button-cancel" onClick={() => setPendingDeleteTransfer(null)}>Cancel</button><button type="button" className="budget-card-button budget-card-button-delete" onClick={() => { if (onDeleteTransfer?.(pendingDeleteTransfer.id)) { setPendingDeleteTransfer(null); setSelectedTransaction(null) } }}><FontAwesomeIcon icon={faTrashCan} aria-hidden="true" />Delete transfer</button></div>
      </div></div>}
      {pendingDeleteExpense && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="delete-expense-title"><div className="add-form edit-form"><div className="add-form-header"><h3 id="delete-expense-title">Delete expense?</h3><button className="add-form-close" type="button" onClick={() => setPendingDeleteExpense(null)} aria-label="Close delete confirmation"><FontAwesomeIcon icon={faXmark} aria-hidden="true" /></button></div><p className="delete-confirmation-text">This will remove {pendingDeleteExpense.title} from the selected period.</p><div className="budget-card-actions single-action-row"><button type="button" className="budget-card-button budget-card-button-delete" onClick={confirmDeleteExpense}><FontAwesomeIcon icon={faTrashCan} aria-hidden="true" />Delete</button></div></div></div>}
    </div>
  )
}
