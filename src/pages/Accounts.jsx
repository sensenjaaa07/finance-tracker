...
import { faXmark, faTrashCan, faPenToSquare, faSliders, faArrowRightArrowLeft, faArrowRight } from '@fortawesome/free-solid-svg-icons'
import Header from '../components/Header'
import '../assets/styles/EntryList.css'
import '../assets/styles/Dashboard.css'

const Accounts = ({ accounts, onOpenAddForm, onUpdateAccount, onAdjustAccountBalance, onDeleteAccount, onTransfer }) => {
  ...
  const totalBalance = accounts.reduce((total, entry) => total + Number(entry.amount ?? 0), 0)
  const selectedFromAccount = accounts.find(account => account.id === transferDraft.from)
  const selectedToAccount = accounts.find(account => account.id === transferDraft.to)
  ...
  const handleSave = () => {
    if (!onUpdateAccount || !editingEntry) return
    if (!draft.name.trim()) return
    if (onUpdateAccount(editingEntry.id, { name: draft.name.trim() })) setEditingEntry(null)
  }
  ...
  const confirmDeleteEntry = () => {
    if (!onDeleteAccount || !pendingDeleteEntry) return
    if (onDeleteAccount(pendingDeleteEntry.id)) setPendingDeleteEntry(null)
  }
  ...
  return (
    <div>
      ...
      <div className="dashboard-summary">
        ...
        <div className="summary-card"><p>Number of accounts</p><strong>{accounts.length}</strong></div>
      </div>

      <div className="daily-budget-section" style={{ marginBottom: '24px' }}>
        <div className="daily-budget-header">
          <div><p className="chart-eyebrow">Account transfers</p><h2>Move money between accounts</h2><p className="daily-budget-description">Transfer money without creating an expense.</p>{accounts.length < 2 && <p className="daily-budget-description">Add at least two accounts to enable transfers.</p>}</div>
          <button type="button" className="budget-card-button budget-card-button-save" onClick={() => setShowTransfer(true)} disabled={accounts.length < 2}><FontAwesomeIcon icon={faArrowRightArrowLeft} aria-hidden="true" />Transfer money</button>
        </div>
      </div>

      <div className="entry-list">
        {accounts.length === 0 ? <p className="empty-state">Add an account to start tracking your money.</p> : accounts.map((entry) => (
          <div className="entry-list-item info-card" key={entry.id}>
            <div className="info-card-header"><p className="info-card-label">Account</p><div className="budget-card-header-actions"><span className="budget-card-badge">Active</span><button type="button" className="budget-card-delete-button" aria-label={`Delete ${entry.name}`} onClick={() => setPendingDeleteEntry(entry)}><FontAwesomeIcon icon={faTrashCan} aria-hidden="true" /></button></div></div>
            <h3>{entry.name}</h3>
      ...
      {showTransfer && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="transfer-title">
        <form className="add-form edit-form transfer-modal-card" onSubmit={handleTransfer}>
          <div className="transfer-modal-heading">
            <div className="transfer-modal-icon" aria-hidden="true"><FontAwesomeIcon icon={faArrowRightArrowLeft} /></div>
          ...
          <div className={`transfer-summary ${hasTransferDetails ? 'is-ready' : ''}`}><div className="transfer-summary-icon" aria-hidden="true"><FontAwesomeIcon icon={faArrowRightArrowLeft} /></div><div><strong>Transfer Summary</strong>{hasTransferDetails ? <p>₱{transferAmount.toFixed(2)} will move from <b>{selectedFromAccount.name}</b> to <b>{selectedToAccount.name}</b>.</p> : <p>Select both accounts and enter an amount to continue.</p>}</div></div>
          <div className="budget-card-actions edit-action-row transfer-modal-actions"><button type="button" className="budget-card-button budget-card-button-cancel" onClick={closeTransfer}><FontAwesomeIcon icon={faXmark} aria-hidden="true" />Cancel</button><button type="submit" className="budget-card-button budget-card-button-save" disabled={!hasTransferDetails}><FontAwesomeIcon icon={faArrowRightArrowLeft} aria-hidden="true" />Transfer money</button></div>
        </form>
      </div>}

      {editingEntry && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="edit-account-title"><div className="add-form edit-form"><div className="add-form-header"><h3 id="edit-account-title">Edit account name</h3><button className="add-form-close" type="button" onClick={() => setEditingEntry(null)} aria-label="Close edit form"><FontAwesomeIcon icon={faXmark} aria-hidden="true" /></button></div><div className="add-form-fields"><label className="add-form-label" htmlFor="account-edit-name">Account name</label><input id="account-edit-name" value={draft.name} onChange={(event) => setDraft(previous => ({ ...previous, name: event.target.value }))} /><p className="add-form-helper">Current balance: ₱{Number(editingEntry.amount ?? 0).toFixed(2)}</p><div className="budget-card-actions single-action-row"><button type="button" className="budget-card-button budget-card-button-save" onClick={handleSave}><FontAwesomeIcon icon={faPenToSquare} aria-hidden="true" />Save name</button></div></div></div></div>}

      {adjustingEntry && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="adjust-balance-title"><div className="add-form edit-form"><div className="add-form-header"><div><p className="budget-reallocation-eyebrow">Balance adjustment</p><h3 id="adjust-balance-title">Adjust {adjustingEntry.name}</h3></div><button className="add-form-close" type="button" onClick={() => setAdjustingEntry(null)} aria-label="Close balance adjustment"><FontAwesomeIcon icon={faXmark} aria-hidden="true" /></button></div><div className="add-form-fields"><div className="budget-card-metrics"><div><span className="budget-card-label">Current balance</span><p>₱{Number(adjustingEntry.amount ?? 0).toFixed(2)}</p></div><div><span className="budget-card-label">New balance</span><p className={adjustedBalancePreview < 0 ? 'amount-negative' : ''}>₱{Math.max(0, adjustedBalancePreview).toFixed(2)}</p></div></div><label className="add-form-label" htmlFor="adjustment-direction">Adjustment</label><select id="adjustment-direction" value={adjustmentDraft.direction} onChange={(event) => setAdjustmentDraft(previous => ({ ...previous, direction: event.target.value }))}><option value="increase">Add money</option><option value="decrease">Remove money</option></select><label className="add-form-label" htmlFor="adjustment-amount">Amount</label><div className="transfer-amount-input"><span>₱</span><input id="adjustment-amount" type="number" min="0.01" step="0.01" value={adjustmentDraft.amount} onChange={(event) => setAdjustmentDraft(previous => ({ ...previous, amount: event.target.value }))} placeholder="0.00" /></div><p className="add-form-helper">Use this for starting-balance corrections or cash adjustments. It does not count as income or an expense.</p><div className="budget-card-actions edit-action-row"><button type="button" className="budget-card-button budget-card-button-cancel" onClick={() => setAdjustingEntry(null)}><FontAwesomeIcon icon={faXmark} aria-hidden="true" />Cancel</button><button type="button" className="budget-card-button budget-card-button-save" onClick={handleAdjust} disabled={!Number.isFinite(adjustmentAmount) || adjustmentAmount <= 0 || adjustedBalancePreview < 0}><FontAwesomeIcon icon={faSliders} aria-hidden="true" />Adjust balance</button></div></div></div></div>}

      {pendingDeleteEntry && <div className="expense-card-overlay" role="dialog" aria-modal="true" aria-labelledby="delete-account-title"><div className="add-form edit-form"><div className="add-form-header"><h3 id="delete-account-title">Delete account?</h3><button className="add-form-close" type="button" onClick={() => setPendingDeleteEntry(null)} aria-label="Close delete confirmation"><FontAwesomeIcon icon={faXmark} aria-hidden="true" /></button></div><p className="delete-confirmation-text">This removes <strong>{pendingDeleteEntry.name}</strong>. Accounts with transaction history cannot be deleted.</p><div className="budget-card-actions edit-action-row"><button type="button" className="budget-card-button budget-card-button-cancel" onClick={() => setPendingDeleteEntry(null)}><FontAwesomeIcon icon={faXmark} aria-hidden="true" />Cancel</button><button type="button" className="budget-card-button budget-card-button-delete" onClick={confirmDeleteEntry}><FontAwesomeIcon icon={faTrashCan} aria-hidden="true" />Delete account</button></div></div></div>}
    </div>
  )
}
