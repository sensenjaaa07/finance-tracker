...
const Navigation = () => {
  ...
  return (
    <>
      <button type="button" className={`mobile-nav-toggle${isOpen ? ' mobile-nav-toggle-open' : ''}`} onClick={() => setIsOpen(previous => !previous)} aria-label={isOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={isOpen}>
      ...
      <aside className={`navigation-container${isOpen ? ' mobile-nav-open' : ''}`}>
        <div className="logo-container">
          ...
          <div><p className="logo-eyebrow">Personal</p><h2 className="nav-title">Finance Tracker</h2></div>
        </div>
